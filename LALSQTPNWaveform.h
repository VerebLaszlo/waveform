 ...
 *	[2] Alessandra Buonanno, Yanbei Chen, and Michele Vallisneri, Phys.Rev. D67 (2003)<br />
 *	[3] Balázs Mikóczi, Mátyás Vasúth, László Á. Gergely, Phys.Rev. D71 (2005)
 * @author László Veréb
 * @date 2010.05.21.
 */
...
#define LALSQTPN_ENULL 1
#define LALSQTPN_MSGWAVEPARAMSNULL "the parameter structure of the waveform is null"
#define LALSQTPN_ENNULL 2
#define LALSQTPN_MSGWAVESTRUCTNULL "the waveform structure is null"
#define LALSQTPN_MSGWAVESTRUCTNNULL "the waveform structure is not null"
...
typedef enum {
	...
	LALSQTPN_LALSTPNComp = 3, ///< combined: spin-orbit and spin-spin component together, like the LALSTPNWaveform
	LALSQTPN_AllComp = LALSQTPN_SOComp | LALSQTPN_SSComp | LALSQTPN_SSselfComp | LALSQTPN_QMComp	///< all components
...
typedef struct tagLALSQTPNWaveformParams{
	...
	//@{
	REAL8 mass[3]; ///< masses of the BHs in \f$M_\odot\f$
	...
	//@{
	REAL8 chi[3][3]; ///< components of the normalized spin
	REAL8 chih[3][3]; ///< components of the unity-vectors of the normalized spin
	REAL8 chiAmp[3]; ///< amplitude of the normalized spin
	//@}
	///@name other system-parameters
	//@{
	REAL8 flatness[3]; ///< flatness of the BHs or NSs
 ...
 *		\end{gathered}\\\hline
 *		\derTpM{\hat{L_N}}=\sum_i-\frac{1}{\eta}\frac{\chi_im_i^2}{M^2}\derTpM{\hat{\chi_i}},
 *		\end{gather}\hline
 *		\begin{gather}
 *		\begin{gathered}
 *			\begin{split}
 *			\derTpM{\left(M\omega\right)}&=\frac{96\eta}{5}\OM{11}\bigg[1-\frac{743+924\eta}{336}\OM{2}\bigg.\\&\quad+\left(4\pi+SO_{\omega}\right)\OM{3}+\left(\frac{34103}{18144}+\frac{13661}{2016}\eta+\frac{59}{18}\eta^2\bigg.\\&\quad+\bigg.\bigg.SS_{\omega}+SSself_{\omega}+QM_{\omega}\bigg)\OM{4}\bigg],
 *			\end{split}\\
 ...
 *		\begin{gathered}
 *			\begin{split}
 *			MECO&=-0.5\eta\frac{2}{3}\OM{-1}+0.5\eta\frac{4}{3}\frac{9+\eta}{12}\OM{1}\\&\quad+SO_{MECO}\OM{2}+\Big(0.5\eta\frac{6}{3}\frac{81-57\eta+\eta^2}{24}\Big.\\&\quad+\Big.SS_{MECO}+QM_{MECO}\Big)\OM{3},
 *			\end{split}\\
 ...
 *		\end{gathered}\\\hline
 *		\begin{gathered}
 *		\derTpM{\Phi}=M\omega-\derTpM{\alpha}\cos\iota\\
 *		\alpha=\arctan\dfrac{\hat{L_N}_y}{\hat{L_N}_x}
 *		\end{gathered}
 *		\end{gather}
 *	\f}
 ...
 *	equation (4) is given by (Eq. (11),(13)) of [2] except of the quadropole-monopole contribution (\f$QM_{MECO}\f$) that was added here, equation
 *	(5) is given by (Eq. (4.31)) of [1].<br />
 */
/**	The equation's constant parts are calculated by the LALSQTPNFillCoefficients() function. This function is used by the LALSQTPNIntegratorFunc() function to evolve the system with a given time.
 ...
 *	The function returns the \f$a_1\f$, \f$a_2\f$ amplitude, \f$\alpha\f$ polarization
 *	shift, and \f$\Phi\f$ phase (Eq. (4.28) of [1] upto leading order.
 *	We note that \f$\Theta=0\f$ in leading order because we use radiation frame).
