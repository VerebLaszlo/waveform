...
/**		The macro function calculates the weighted vector product of two vectors.
...
void LALSQTPNFillCoefficients(LALStatus *status, LALSQTPNWaveformParams * const params) {
	...
	switch (params->order) {
		...
		case LAL_PNORDER_TWO:
			...
			if ((params->spinComponent & LALSQTPN_SSComp) == LALSQTPN_SSComp) {
			...
			if ((params->spinComponent & LALSQTPN_SSselfComp) == LALSQTPN_SSselfComp) {
				for (i = 0; i < 2; i++) {
			...
			if ((params->spinComponent & LALSQTPN_QMComp) == LALSQTPN_QMComp) {
				for (i = 0; i < 2; i++) {
					...
					params->coeff.dchihQM[i] = -params->flatness[i] * params->eta
							* params->chiAmp[i] * 3. / 2.;
				}
		...
		case LAL_PNORDER_ONE_POINT_FIVE:
			if (params->spinComponent != 0) {
				...
				for (i = 0; i < 2; i++) {
					...
					dvalues[LALSQTPN_MECO] += params->coeff.mecoSO[i] * LNhchih[i]
							* omegaPowi_3[2];
				}
...
void LALSQTPNGenerator(LALStatus *status, CoherentGW *waveform, LALSQTPNWaveformParams *params) {
	...
	TRY(LALSQTPNFillCoefficients(status->statusPtr, params), status);
	...
	while (dvalues[LALSQTPN_MECO] < 0. && dvalues[LALSQTPN_OMEGA] > 0.0 && SQR(values[LALSQTPN_LNH_3])
		...
		TRY(LALSQTPNIntegratorFunc(status->statusPtr, values, &integrator, step), status);
