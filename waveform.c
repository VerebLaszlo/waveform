	...
	case LAL_PNORDER_TWO:
		...
		for (i = 0; i < 2; i++) {
			...
			params->coeff.SS_const += spin_MPow2[i] * params->chi_Amp[i] * 7.
					/ 96.;
			params->coeff.QM = spin_MPow2[i] * params->chi_Amp[i] * flatness[i]
					* 7.5;
...
void derivator(double t, const double values[], double dvalues[], void * params) {
	...
	for (i = LAL_PNORDER_NEWTONIAN; i >= LAL_PNORDER_PSEUDO_FOUR; i++) {
		domega += params->coeff.domega[i] * omegaPowi_3[i];
	}
	switch (order) {
	case LAL_PNORDER_PSEUDO_FOUR:
	case LAL_PNORDER_THREE_POINT_FIVE:
	case LAL_PNORDER_THREE:
		domega += params->coeff.ln_coeff * log(16. * omegaPowi_3[2])
				* omegaPowi_3[LAL_PNORDER_THREE];
	case LAL_PNORDER_TWO_POINT_FIVE:
	case LAL_PNORDER_TWO:
		double QM = params->coeff.QM_const;
		double SS = params->coeff.SS_const;
		double S1S2 = params->coeff.S1S2[0] * LNhchih[0] * LNhchih[1]
				+ params->coeff.S1S2[1] * chih1chih2;
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 2; j++) {
//				dchih[j][i] += params->coeff.dchih[j][LAL_PNORDER_TWO] * ;
			}
		}
		for (i = 0; i < 2; i++) {
			SS += params->coeff.SS[i] * LNhchih[i] * LNhchih[i];
			QM += params->coeff.QM[i] * LNhchih[i];
		}
		domega += (QM + SS + S1S2) * omegaPowi_3[LAL_PNORDER_TWO];
	case LAL_PNORDER_ONE_POINT_FIVE:
		for (i = 0; i < 2; i++) {
			domega += params ->coeff.SO[i] * LNhchih[i] *
			omegaPowi_3[LAL_PNORDER_ONE_POINT_FIVE];
		}
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 2; j++) {
				dchih[j][i] +=
				params->coeff.dchih[i][LAL_PNORDER_ONE_POINT_FIVE] * LNhxchih[j]
				* omegaPowi_3[LAL_PNORDER_ONE_POINT_FIVE];
				dLNh[i] += params->coeff.dLNh[j] * dchih[j][i];
			}
			dLNh[i] *= omegaPowi_3[0];
		}
	case LAL_PNORDER_ONE:
	case LAL_PNORDER_HALF:
	case LAL_PNORDER_NEWTONIAN:
